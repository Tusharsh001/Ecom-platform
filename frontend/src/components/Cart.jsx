...
const Cart = () => {
  ...
  // Sync cartItems directly from context cart
  useEffect(() => {
    setCartItems(cart);
  }, [cart]);
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        const { imageName, imageData, imageType, quantity, ...rest } = item;
        const updatedStockQuantity = item.stockQuantity - item.quantity;
        const updatedProductData = { ...rest, stockQuantity: updatedStockQuantity };

        // Convert existing base64 imageData back to file
        const byteArray = Uint8Array.from(atob(imageData), (c) => c.charCodeAt(0));
        const blob = new Blob([byteArray], { type: imageType });
        const file = new File([blob], imageName || "image.jpg", { type: imageType });

        const cartProduct = new FormData();
        cartProduct.append("imageFile", file);
        cartProduct.append(
          "product",
